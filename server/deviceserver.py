...
import settings
from settings import ADPNS_with_db,db_tbs
import utils
from db import session,dba_logger,tables
...
use_dbapi = True
from libs.dbapi import dbquery,dbupdate,dbwrite
...
class Connection(object):
    ...
    def send_message(self, mess_id,sendno,data):
        ...
        if re.search('{.+?}\*#\*',data) != None:
            ...
            if utils.checkjsonkey(['appkey','device','action'],json_obj):
                ...
                self._response = msg_format % ('*'*1+'*#*',self._devicetoken)
                ...
                if ADPNS_with_db:
                    if debug2:
                        logger.log(10, 'one entry start point : %s' % datetime.datetime.now())
                    try:
                        if use_dbapi:
                            #res = dbquery(tablename='sessions_details',whereclause=and_(connection_writer._table.c.app_key==self.app_key,connection_writer._table.c.device_token==self._devicetoken))
                    ...
                    if res.first() is None:
                        if json_obj['action'] == 'reg':
                            if use_dbapi:
                                dbwrite(tablename='sessions_details',app_key=self.app_key,device_token=self._devicetoken)
                            else:
                        ...
                        if json_obj['action'] =='reg':
                            ...
                            if use_dbapi:
                                #dbupdate(tablename='sessions_details',whereclause=and_(connection_writer._table.c.app_key==self.app_key,connection_writer._table.c.device_token==self._devicetoken),create_time=tmptime,end_time=tmptime,session_status=self.status)
                                dbupdate(tables['sessions_details'],whereclause=and_(tables['sessions_details'].c.app_key==self.app_key,tables['sessions_details'].c.device_token==self._devicetoken),create_time=tmptime,end_time=tmptime,session_status=self.status)
                            else:
                            ...
                            #tmptime = datetime.datetime.now()
                            if use_dbapi:
                                #dbupdate(tablename='sessions_details',whereclause=and_(connection_writer._table.c.app_key==self.app_key,connection_writer._table.c.device_token==self._devicetoken),session_status=self.status)
                                dbupdate(tables['sessions_details'],whereclause=and_(tables['sessions_details'].c.app_key==self.app_key,tables['sessions_details'].c.device_token==self._devicetoken),session_status=self.status)
                            else:
                        ...
                        if self.status is None:
                            ...
                            return
                    if debug2:
                        logger.log(10, 'one entry end point : %s' % datetime.datetime.now())
                else:
                    #need to justify '0' or '1'
                    self.status = '0'    
    ...
    def on_close(self):
        ...
        if hasattr(self,'app_key') and hasattr(self,'_devicetoken'):
            ...
            if ADPNS_with_db:
                if use_dbapi:
                    #dbupdate(tablename=db_tbs['connection'],whereclause=and_(connection_writer._table.c.app_key==self.app_key,connection_writer._table.c.device_token==self._devicetoken),end_time=tmptime,session_status='1')
                    dbupdate(tables['sessions_details'],whereclause=and_(tables['sessions_details'].c.app_key==self.app_key,tables['sessions_details'].c.device_token==self._devicetoken),end_time=tmptime,session_status='1')
                else:
    ...
    def genMsgToDevice(self,id,msg_dict):
    ...
    def feed_ok(self,mess_id,data):
        ...
        if ADPNS_with_db:
            if use_dbapi:
                dbwrite(tablename=db_tbs['message_detail'],mess_id=mess_id,device_token=self._devicetoken,mess_status='0')
            else:
    ...
    def feedback_timeout_handle(self,sendno,mess_id):
        if self.getTry(sendno) < settings.retry_times:
            ...
            if ADPNS_with_db:
                if use_dbapi:
                    dbwrite(tablename=db_tbs['message_detail'],mess_id=mess_id,device_token=self._devicetoken,mess_status='1')
                else:
...
def pushToDevice(message):
    ...
    if mess_id not in mess_ids:
        if ADPNS_with_db:
            if use_dbapi:
                dbwrite(tablename=db_tbs['message'],mess_id=mess_id, mess_context=mess_context)
            else:
